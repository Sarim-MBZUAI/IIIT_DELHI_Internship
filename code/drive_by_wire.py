...
def main():
    ...
    while not initialized:
        try:
            # open('COM8')
            ser = serial.Serial('COM8', 9600)  # steer and break arduino port
            # ser_throttle = serial.Serial("COM8", 9600)  # throttle arduino port
            initialized = True
            print("serial up")
            # print(ser.read())
        except:
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                elif (event.key == pygame.K_SPACE or event.key == pygame.K_b):  # front break diseanagae
                    ser.write(b'b')
                    print("pressed space")

                elif(event.key == pygame.K_n): #engage
                    ser.write(b'B')
                    print("n -> reverse front break")

                elif(event.key == pygame.K_KP_PERIOD or event.key == pygame.K_KP5):  # rear break
                    ser.write(b'z')
